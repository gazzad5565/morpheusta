...
import { CustomerScopePicker } from "@/components/ui/CustomerScopePicker";
...
function NewTaskPage() {
  ...
  // Feature D (May 13) — signature on tasks. When true, the rep must
  // capture a customer signature on the rep app before they can mark
  // the task complete. Combines with photo gates: both must pass
  // when both are set.
      ...
      >
        <Card padding={20}>
          ...
          <Field label="Applies to" required>
            {/* Shared CustomerScopePicker (May 28) — gives Tasks the
                same All / Specific / by-region / by-group selection as
                Library + schedule. value null = universal (one row,
                customer_id NULL); string[] = one row per customer. */}
            <CustomerScopePicker
              customers={customers}
              loading={loading}
              value={scope === "all" ? null : Array.from(selectedIds)}
              onChange={(next) => {
                if (next === null) {
                  setScope("all");
              ...
              allSubLabel="Universal task"
              specificLabel="Specific customers"
              specificSubLabel="Pick one or many · or by region / group"
            />
          </Field>
          ...
          {/* Signature requirement — Feature D (May 13). Independent of
              photos: a task can require photos only, signature only,
              both, or neither. When both, the rep app's chain goes
              photos → signature → auto-complete. */}
          <div style={{ marginBottom: 14 }}>
            <Field
