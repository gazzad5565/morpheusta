...
0. **Import hub + email welcome — Phases A + B + C landed May 25, D + E to ship.** Foundation in (Phase A): `import_runs` table, `geocode_status` on customers + sites, Resend wiring, `/settings/import` page. First user-visible feature in (Phase B): "Email this user" button on `/settings/managers/[id]/edit` + `/reps/[id]` actions slot. Second user-visible feature in (Phase C): `/settings/import` hub (tabbed: "Run an import" + "Defaults") + 5-step `/settings/import/[entity]` stepper (Source → Map → Settings → Preview → Result) for all five entity types, consolidated `Import` CTAs across `/customers`, `/reps`, `/settings/managers`, `/schedule`, and the customer-detail `SitesTab`. All five adapters' Phase-C stubs throw "Phase D not wired up" on Commit — the wizard surfaces that cleanly on the Result step. **Reorg note (same day):** Import is reachable only via Sidebar → Settings → Import (no top-level nav entry, no top-level `/import` route). List-page Import buttons remain as shortcuts that funnel to the same hub.
