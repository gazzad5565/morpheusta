...
0a-old. **✅ DONE (Jun 1) — `RESEND_FROM` set in Vercel; emails now deliver to anyone.** Set `RESEND_FROM = "Morpheus Ops <noreply@morpheusops.app>"` on the morpheus-admin **Production** env via the Vercel CLI (Gary confirmed `morpheusops.app` is verified in Resend), then redeployed (deploy `qm272ncmb`, ● Ready) so the serverless functions pick it up. `RESEND_API_KEY` was already set. (Preview env NOT set — the CLI's non-interactive "all preview branches" prompt blocked it; add via the Vercel dashboard if preview deploys ever need to email. Production — the live app — is covered.) Background, now historical: the EmailUserModal on /settings/managers/[id]/edit and /reps/[id] previously fell back to Resend's `onboarding@resend.dev` sandbox sender, which only delivered to the Resend account holder's own email (gazzad@mac.com). Ruled-out causes (kept for the record):
...
0. **Import hub + email welcome — Phases A → E all landed May 25.** All five phases shipped same day. Workstream complete; remaining items are operator setup.
   ...
   **Operator checklist to make the full pipeline live:**
   ...
   - Add `RESEND_API_KEY` to the morpheus-admin Vercel project (Production + Preview + Development). Once `morpheusops.app` is verified as a sending domain in Resend, also set `RESEND_FROM=Morpheus Ops <hello@morpheusops.app>` (or whichever local-part you pick) so bulk rep emails can deliver to any recipient.
   - Confirm `CRON_SECRET` is set on the morpheus-admin Vercel project (already there from the existing `/api/cron/messages` etc — verify before relying on the geocoder cron).
   - Confirm `NEXT_PUBLIC_ADMIN_URL` + `NEXT_PUBLIC_MOBILE_URL` are in the Supabase Auth Redirect URLs allowlist (Authentication → URL Configuration) so the recovery-link path lands users in the right app.
   - End-to-end smoke test: open `Settings → Import → Customers`, drop the `customers.csv` template, walk the wizard, commit. 3 rows should land in `/customers`. Within 60s the geocoder cron resolves their addresses and they appear on the Live Ops map. Repeat with the reps template (welcome-email on) to verify Resend delivery.

   **What shipped (each phase has a SESSIONS.md subsection):**
   - **Phase A** — `import_runs` table, `geocode_status` columns on customers + sites, Resend wiring (`lib/email.ts`, `WelcomeEmail.tsx`, `/api/email/test`), `/settings/import` defaults page.
   - **Phase B** — "Email this user" button on manager edit + rep detail pages, `EmailUserModal` with invite-link and regenerate-password flows, `profiles.last_credentials_sent_at` column.
   - **Phase C** — `/settings/import` tabbed hub (Run / Defaults) + 5-step `/settings/import/[entity]` wizard (Source → Map → Settings → Preview → Result), synonyms registry, Papa Parse + SheetJS file parsers, sample CSV templates, consolidated Import CTAs on every list page funnelling to the one hub.
   - **Phase D** — real entity adapters in `lib/import-adapters/` (customer, site, rep+manager, shift with weekly expansion), `/api/import/users` server route for user creation + welcome-email send, wizard onCommit writes `import_runs` + `shift_events` rows.
   - **Phase E** — `/api/cron/geocode-queue` worker (every minute, drains pending rows with 1.1s Nominatim sleep), `GeocodeBadge` UI component, `updateCustomer`/`updateSite` retry-on-address-edit logic, `lib/geocode-server.ts` shared helper.
