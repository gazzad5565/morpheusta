...
4. ~~**Phase 4 RLS**~~ ✅ Migration written: `db/migrations/2026_05_14_phase4_rls_hardening.sql`. **Run it in Supabase SQL editor** before opening to real users — single coordinated rewrite via `is_manager()` SECURITY DEFINER helper, applied to 19 tables + 3 storage buckets. Idempotent. Smoke-test checklist at the bottom of the file.
...
- ✅ **Phase 4 RLS hardening — comprehensive migration written.** `db/migrations/2026_05_14_phase4_rls_hardening.sql` introduces an `is_manager()` SECURITY DEFINER helper and re-scopes every previously-permissive policy across 19 tables + 3 storage buckets. Idempotent. **Needs to be run in Supabase SQL editor before opening to real users.** Smoke-test checklist included in the file.
