...
> Latest commit: **`ed14a0a`** (May 21, 2026). Admin-side photo viewer finally landed — mobile's been writing `shift_task_photos` rows + uploading JPEGs to the public `shift_photos` bucket since May 13 (Feature C), but admin had no UI to read them. New `morpheus-admin/lib/photos-store.ts`, inline 64×64 thumbnails per task row on `/shifts/[id]`, full-screen `PhotoLightbox` that flicks through every photo for the shift (backdrop / Esc / ← → / × all close + nav, wrap-around prev/next, caption "Photo N of M · task · time"), and a small `📷 N` chip on each shift row in `/past-shifts` (Table + Grid) + Live Ops Today's Shifts (omitted at 0 so rows stay clean). New `camera` glyph in `AGlyph`. Same day also merged the `260519-UIFixes` branch in (`cdfde3d`) — customer detail page split out of its 1,293-line monolith into proper tab components (`OverviewTab` / `ContactsTab` / `RepsTab` / `LibraryTab` / `TasksTab` / `ShiftsTab` + shared `tabStyles.ts`), SiteEditor + SiteRow extracted from SitesTab, new `/past-shifts` archive page with Grid + Table views, new admin UI primitives (`EmptyState`, `ExpandableRow`, `Pill`, `TabHeader`), Combobox + Avatars + CustomFieldsCard uplift, Playwright coverage added for the new flows. Plus `MARKETING_BRIEF.md` (`e690495`) at repo root — canonical product reference for the Marketing Muse agent stack. Before that, **`b1a739b`** (May 15, overnight) was a sidebar design pass on top of the May 14 work. Bigger nav links, "In real time" rendered as a brand-cyan pill with the existing shimmer kept, soft fade-to-warmer gradient on the dark sidebar's bottom, Tasks sub-nav now toggles + animates with a `cubic-bezier(.22, 1, .36, 1)` open/close, and a new **org name accent colour picker** at /settings/organisation that paints the wordmark in any hex (instant repaint via the existing `morpheus.org.changed` event, no reload). Then the May 14 work — Phase 4 RLS pass shipped + applied to prod + the long polish + bug-fix day. ~50 commits total across the rolling session in three groups:
