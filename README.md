...
> Latest: **May 27, 2026 — pagination on every long list page.** New shared `components/ui/Pagination.tsx` (First / Previous / page numbers / Next / Last + "Showing X-Y of Z"). Wired into `/tasks`, `/library`, `/reps`, `/settings/managers`, and `/customers`. Page size 50; client-side slicing of already-filtered arrays so all existing search/filter/sort behaviour is unchanged. Customers Map view bypasses pagination intentionally (pin overview shouldn't be paginated). See [docs/SESSIONS.md](docs/SESSIONS.md) top entry.
>
