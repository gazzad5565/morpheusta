...
> Latest: **May 27, 2026 (late) — rep types + capability flags.** Admin-managed vocabulary in `app_settings.rep_types` + `profiles.rep_type` column. First flag: `canCreateCustomers` — drives whether the mobile Add Customer affordance shows. New "Manage rep types" modal on `/settings/managers`, dropdown on the user edit page, filter `<select>` on `/reps`, optional `rep_type` column on the user import adapter (validated server-side against the live vocabulary), mobile SideMenu hides Add Customer when capability is false, `/add-customer` belt-and-braces block screen on deep-link nav. Migration `2026_05_27_profiles_rep_type.sql` PENDING. Client-side enforcement only (RLS hardening deferred — flagged in SESSIONS.md).
>
